...
public class SecuritySmsValidationFilter extends OncePerRequestFilter implements InitializingBean {
    ...
    private void validate(@NotNull ServletWebRequest servletWebRequest) throws VerificationCodeException, ServletRequestBindingException {
        SmsCode smsCode = (SmsCode) sessionStrategy.getAttribute(servletWebRequest, "SESSION_KEY_SMS_CODE");

        SmsCode smsCode = (SmsCode) = HttpSession session = servletWebRequest.getRequest().getSession();
        ...
        if (smsCode.expired()) {
            sessionStrategy.removeAttribute(servletWebRequest, "SESSION_KEY_SMS_CODE_");
